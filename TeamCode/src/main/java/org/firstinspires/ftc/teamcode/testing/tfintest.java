...
public class tfintest extends OpMode {
    ...
    private static final GamepadKeys.Button
            Launch_Button=GamepadKeys.Button.CROSS,
            Intake_Toggle=GamepadKeys.Button.TRIANGLE,
            Revolver_Cw=GamepadKeys.Button.SQUARE,
            Revolver_Ccw=GamepadKeys.Button.CIRCLE,
            Fire_Button=GamepadKeys.Button.BACK;
    enum ShootState{
        IDLE,
        AIMING,
        SPINNING,
    ...
    public void intake() {
        ...
        if (gamepadEx.wasJustPressed(Intake_Toggle)) {
    ...
    public void rotate() {
        ...
        if (eject.getAngle() != 51) {
            if (gamepadEx.wasJustPressed(Revolver_Cw)) {
                revolverTarget += Globals.revolver.oneRotation;  // CW
            } else if (gamepadEx.wasJustPressed(Revolver_Ccw)) {
...
//        prevTri = tri;
        ...
        boolean lb = gamepadEx.wasJustPressed(GamepadKeys.Button.LEFT_BUMPER);
        boolean rb = gamepadEx.wasJustPressed(GamepadKeys.Button.RIGHT_BUMPER);
    ...
    private void launcherawe() {
        ...
        if (gamepadEx.wasJustPressed(Launch_Button)) {
    ...
    private void shootSequence() {
        switch (shootState) {
            case IDLE:
                if (gamepadEx.wasJustPressed(Fire_Button)) {
            ...
            case AIMING:
                ...
                List<AprilTagDetection> detections = tagProcessor.getDetections();
                if (detections != null && !detections.isEmpty()) {
                    distance = detections.get(0).ftcPose.range;
                }
            ...
            case SPINNING:
                double targetRPM = (2547.5 * Math.pow(Math.E, (0.0078 * distance)));
                double ffPower = ff.calculate(RPM, targetRPM);
                launcher1.set(ffPower);
                launcher2.set(ffPower);
                calculateRPM();

                if (Math.abs(RPM - targetRPM) < Globals.launcher.launcherTol) {
