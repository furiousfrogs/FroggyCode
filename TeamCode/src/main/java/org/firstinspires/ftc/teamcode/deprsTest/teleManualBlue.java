...
public class teleManualBlue extends OpMode {
    ...
    private String senseColour() {
        if (distanceSensor.getDistance(DistanceUnit.CM) > 3 && secondDistanceSensor.getDistance(DistanceUnit.CM) > 3)
            return "EMPTY";
    ...
    private SpamState spamState = SpamState.IDLE;
    private int spamShotsDone = 0;
    private ElapsedTime spamTimer = new ElapsedTime();

    private void spam() {

        // --- toggle spammer on CROSS rising edge -----
        boolean cross = gamepadEx2.getButton(GamepadKeys.Button.CROSS);
        if (cross && !prevCrossSpammer) {
            if (spamState == SpamState.IDLE) {
                spamState = SpamState.EXTEND_EJECT;
                spamShotsDone = 0;
                spamTimer.reset();
            }
        }
        prevCrossSpammer = cross;
        ...
        switch (spamState) {
            ...
            // --- STEP 1: extend eject servo ---
            case EXTEND_EJECT:
                eject.turnToAngle(Globals.pushServo.eject);
                spamState = SpamState.WAIT_EXTEND;
                spamTimer.reset();
                break;

            // --- STEP 2: wait for sensor to confirm extension ---
            case WAIT_EXTEND:
//                if (ejectAnalog.getVoltage() > 2.3) {
//                    spamState = SpamState.RETRACT_EJECT;
//                    spamTimer.reset();
//                }
                if (spamTimer.seconds() > 0.4) {  // fail-safe
                    spamState = SpamState.RETRACT_EJECT;
                    spamTimer.reset();
                }
                break;

            // --- STEP 3: retract eject servo ---
            case RETRACT_EJECT:
                eject.turnToAngle(Globals.pushServo.defualt);
                if(spamTimer.seconds()>0.5) {
                    spamState = SpamState.ROTATE_REVOLVER;
                    spamTimer.reset();
                }
                break;

            // --- STEP 4: rotate revolver one cell ---
            case ROTATE_REVOLVER:
                previousRevolverPosition = revolver.getCurrentPosition();
                revolverReady = false;

                // one rotation ONLY ONCE
                revolverTarget += shootCounterClockwise
                        ? Globals.revolver.oneRotation
                        : -Globals.revolver.oneRotation;

                spamState = SpamState.WAIT_ROTATION;
                break;

            // --- STEP 5: wait for rotation to finish ---
            case WAIT_ROTATION:
                revolverPower = revolverPID.calculate(revolver.getCurrentPosition(), revolverTarget);
                revolver.set(revolverPower);

                boolean rotDone = Math.abs(
                        Math.abs(revolver.getCurrentPosition() - previousRevolverPosition)
                                - Globals.revolver.oneRotation) < 4;

                if (!revolverReady && rotDone) {
                    revolverReady = true;
                    spamState = SpamState.SPINUP_LAUNCHER;
                    spamTimer.reset();
                }

                if (spamTimer.seconds() > 0.7) {  // timeout safety
                    spamState = SpamState.SPINUP_LAUNCHER;
                    spamTimer.reset();
                }
                break;

            // --- STEP 6: spin up flywheel ---
            case SPINUP_LAUNCHER:
                launcher1.set(feedforwardPower);
                launcher2.set(feedforwardPower);

                boolean goodRPM = Math.abs(power - RPM) < Globals.launcher.launcherTol;

                if (goodRPM) {
                    spamState = SpamState.FIRE;
                    spamTimer.reset();
                }

                if (spamTimer.seconds() > 0.8) { // fail-safe
                    spamState = SpamState.FIRE;
                    spamTimer.reset();
                }
                break;
            ...
            case FIRE:
                ...
                if (spamTimer.seconds() > 0.12) {
                    spamState = SpamState.RESET_LAUNCHER;
                    spamTimer.reset();
                }
                break;

            // --- STEP 8: reset servo + decide next shot ---
            case RESET_LAUNCHER:
                set.turnToAngle(Globals.launcher.downset);
                ...
                if (spamTimer.seconds() > 0.12) {
                    ...
                    if (spamShotsDone >= 3) {       // <<<<<<-------- FIXED: 3 SHOTS ONLY
                        launcher1.set(0);
                        launcher2.set(0);
                        spamState = SpamState.IDLE;
                    } else {
                        spamState = SpamState.EXTEND_EJECT;
                    }
                }
